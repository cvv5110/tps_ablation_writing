...
\begin{subequations}
    ...
    \end{align}
\end{subequations}
Note that since the hidden states $\vbeta$ serve as the memory, their initial conditions are set to zero, i.e., $\vbeta(t_0) = \vzero$, no memory at the beginning. The physics-infused model in \cref{eqn_extended_markovian_system} retains the structure of the LCM, while the hidden states account for missing physics through corrections to the stiffness and advection matrices, as well as the forcing term.

\subsubsection{Coupling with Surface Velocity Model}
Provided the physics-infused formulation in \cref{eqn_extended_markovian_system}, the last step in the formulation is to couple it with the SVM to account for the ablative surface recession. To this end, consider the observable of interest as the surface temperature, denoted as $\vz\in\mathbb{R}^{n_z}$, where $n_z$ is the number of monitored surface locations, and collect the resolved and hidden states into one state vector $\vy = \left[\bvu,\vbeta\right]^T\in\mathbb{R}^{n_y}$ with $n_y = N+m$. Define the data-driven observable mapping operator $\vM\in\mathbb{R}^{n_z\times n_{y}}$ and define the PIROM's observable as,
\begin{equation}
    \vz = \vM\vy\label{eqn_pirom_observable_mapping}
\end{equation}

The PIROM is coupled to the SVM in \cref{eqn_svm} by computing the surface recession velocity based on the PIROM's surface temperature prediction $\vz$. Specifically, the surface recession velocity is updated as,
\begin{equation}
    \dot{\vw} = \vXi\vz - \tvf\label{eqn_pirom_svm}
\end{equation}
