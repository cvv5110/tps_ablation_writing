...
\begin{equation}
    \vA(\vu)\dot{\vu} = \left(\vB(\vu) + \vC(t,\vu)\right)\vu + \mathbf{f}(t)\label{eqn_energy_fem}
\end{equation}
where $\vu = \left[\vu^{(1)}, \vu^{(2)}, \ldots, \vu^{(M)}\right]^T\in\mathbb{R}^{MP}$ includes all the DG variables, $\mathbf{f}\in\mathbb{R}^{MP}$ is the external forcing, and the system matrices $\vA$, $\vB$, and $\vC$ are the matrices due to heat capacity, heat conduction, and temperature advection due to mesh motion, respectively. A detailed derivation of \cref{eqn_energy_fem} is provided in Appendix~\hl{DG-FEM}.
...
\hl{note that there is no advection matrix since the temperature approximation requires space derivatives}
